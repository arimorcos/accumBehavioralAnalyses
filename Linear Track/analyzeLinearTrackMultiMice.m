function perf = analyzeLinearTrackMultiMice(mice,outFileName,dateRange)
...
for i = 1:nMice %for each mouse 
    ...
    %run analyzePerf
    if allDates
        [perf.(mice{i}), figHandle] = analyzeLinearTrackOverTime(mice{i},[],[],true);
    else
        [perf.(mice{i}), figHandle] = analyzeLinearTrackOverTime(mice{i},dateRange{i,1},dateRange{i,2},true);
    end
    pause(0.1);
