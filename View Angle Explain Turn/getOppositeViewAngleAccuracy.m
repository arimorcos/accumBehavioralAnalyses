function [accuracy,sig,meanTheta,nTrials] = getOppositeViewAngleAccuracy(dataCell,alpha)
%getOppositeViewAngleAccuracy.m Plots accuracy for opposite view angles
%
%INPUTS
%dataCell
...
for binInd = 1:nBins
    ...
    [trialInd,tempTheta] = findOppositeViewAngleTrials(dataCell,binInd);
    if isempty(trialInd)
        accuracy(binInd) = NaN;
    ...
    pVal = binocdf(nCorrect,nTrials(binInd),...
        1-sum(getCellVals(dataCell,'result.correct==1'))/length(dataCell),...
        'upper');
